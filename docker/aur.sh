...
if [[ ! -z "${aur_packages}" ]]; then
    ...
    curl -o "/tmp/apacman-any.pkg.tar.xz" -L "https://github.com/crtpoms/packages/raw/master/compiled/apacman-any.pkg.tar.xz"
    pacman -U "/tmp/apacman-any.pkg.tar.xz" --noconfirm
    set +e
    apacman -S ${aur_packages} --noconfirm
    exit_code=$?
    set -e
else
    return 0
...
if (( ${exit_code} != 0 && ${exit_code} != 1 )); then
    echo "apacman returned exit code ${exit_code} (exit code 1 ignored), showing info for exit codes:-"
    cat << EOF
0   Success
